...
Trong repository kiểm tra ngày 10/09/2026, chỉ tìm thấy `app/favicon.ico` là icon tam giác mặc định; không có thư mục `src`/`srcc` hoặc file logo riêng của Hanni. Đã bỏ icon mặc định và dùng biểu tượng Hán tự màu đỏ tạm thời ở `app/icon.svg`, đồng bộ header, sidebar và màn đăng nhập qua `Brand`. Khi có file thương hiệu chính thức, thay `app/icon.svg` hoặc cập nhật đường dẫn trong `components/sidebar.tsx`.
...
Ảnh trang công khai chụp thật khi backend chưa chạy; số liệu thiếu hiển thị “—” và nút tải lại.
