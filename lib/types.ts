...
// -------- Đấu đôi 2v2 (dùng chung rating/tier/mùa giải với đấu 1v1) --------

export interface TeamDuelMatched {
  matchId: string;
  /** đồng đội của mình, KHÔNG gồm chính mình — luôn đúng 1 người */
  myTeammates: DuelOpponent[];
  opponentTeam: DuelOpponent[];
  totalRounds: number;
  introMs: number;
}

export interface TeamDuelRoundPayload {
  matchId: string;
  round: number;
  totalRounds: number;
  question: DuelRoundQuestion;
...
export interface TeamDuelRoundResult {
  matchId: string;
  round: number;
  correctIndex: number;
  /** điểm từng người chơi (theo userId), FE tự cộng theo đội từ
   * `myTeammates`/`opponentTeam` thay vì dựa vào thứ tự `teamScores`. */
  scores: Record<string, number>;
  teamScores: [number, number];
}

export interface TeamDuelFinished {
  matchId: string;
  myTeammates: DuelOpponent[];
  opponentTeam: DuelOpponent[];
  myTeamScore: number;
  opponentTeamScore: number;
  winnerIsMyTeam: boolean | null;
  /** ai gây trận kết thúc sớm do rớt mạng, theo góc nhìn người nhận */
  forfeitedBy: "me" | "teammate" | "opponent" | null;
  eloChange: number;
  newElo: number;
}

export interface TeamDuelActiveMatch {
  matchId: string;
  myTeammates: DuelOpponent[];
  opponentTeam: DuelOpponent[];
  totalRounds: number;
